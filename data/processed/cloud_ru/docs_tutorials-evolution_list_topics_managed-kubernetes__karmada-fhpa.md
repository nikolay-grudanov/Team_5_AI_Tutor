---
source_image: docs_tutorials-evolution_list_topics_managed-kubernetes__karmada-fhpa.jpg
page_number: 0
model: model-run-olm-ocr
prompt_type: olmocr_technical
processing_time: 86.83
tokens: 12191
characters: 9266
timestamp: 2025-12-24T05:55:40.848708
finish_reason: stop
---

### Автоматическое масштабирование nginx с FederatedHPA и нагрузочное тестирование с k6

С помощью этого руководства вы реализуете автоматическое горизонтальное масштабирование приложения nginx в мультикластерной среде Karmada с помощью FederatedHPA и проведете нагрузочное тестирование с использованием инструмента k6. Вы получите практические навыки работы с Federated Horizontal Pod Autoscaler, мониторинга метрик, а также анализа масштабирования приложений в Kubernetes кластерах под управлением Karmada.
...
4. Проведите мониторинг процессов автоматизации
...
На этом шаге вы проверите наличие плагина Metrics Server для сбора метрик ресурсов в кластерах-участниках Karmada. Metrics Server необходим для работы FederatedHPA, чтобы автоматизировать масштабирование на основе метрик CPU.

1. Проверьте, что плагин Metrics Server установлен в кластерах mk8s-evo1 и mk8s-evo2. После создания кластера через сервис Managed Kubernetes, Metrics Server устанавливается по умолчанию.
2. Выполните команду для каждого кластера:

```sh
kubectl --kubeconfig=$HOME/join-clusters/evo1 get deployment metrics-server -n kube-system
kubectl --kubeconfig=$HOME/join-clusters/evo2 get deployment metrics-server -n kube-system
```
...
```sh
karmadactl --karmada-context karmada-api server apply -f $HOME/nginx-manifests/nginx-fhpa.yaml
```

3. Убедитесь, что ресурс создан и активен, выполнив команду:

```sh
karmadactl --karmada-context karmada-api server get fhps nginx-fhpa
```
...
```sh
karmadactl --karmada-context karmada-api server describe fhpa nginx-fhpa
```

Вывод содержит историю событий и текущие метрики автоматизации.
...
export const options = {
  stages: [
    { duration: '1m', target: 100 }, // Нарашивание до 100 пользователей за 1 минуту
  ...
  template:
    ...
    spec:
      ...
      - name: k6
        ...
        command: ["k6", "/scripts/load-test.js"]
        volumeMounts:
        - mountPath: /scripts
          name: k6-script
          readOnly: true
...
```sh
kubectl --kubeconfig=$HOME/join-clusters/evo1 get jobs k6-load-test
kubectl --kubeconfig=$HOME/join-clusters/evo1 get pods -l app=k6-load-test
```
...
```sh
watch -n 10 "echo '--- CPU утилизация подов nginx в evo1 ---' && kubectl --kubeconfig=$HOME/join-clusters/evo1 top pod | grep nginx"
```
...
2. Откройте еще одну сессию SSH с VM. Запустите отслеживание статус FederatedHPA:

```sh
watch -n 15 "karmadactl --karmada-context karmada-api server get fhps nginx-fhpa"
```

Вы увидите, как FederatedHPA реагирует на изменение нагрузки и корректирует количество реплик в кластерах-участниках.

3. Откройте еще одну сессию SSH с VM. Запустите отслеживание количества подов nginx:

```sh
watch -n 10 "echo '--- Поды nginx в кластере evo1 ---' && kubectl --kubeconfig=$HOME/join-clusters/evo1 get pods -l app=nginx --no-headers | wc -l"
```
...
```sh
echo "Количество подов nginx в evo1:"
kubectl --kubeconfig=$HOME/join-clusters/evo1 get pods -l app=nginx --no-headers | wc -l
echo "Количество подов nginx в evo2:"
kubectl --kubeconfig=$HOME/join-clusters/evo2 get pods -l app=nginx --no-headers | wc -l
```
