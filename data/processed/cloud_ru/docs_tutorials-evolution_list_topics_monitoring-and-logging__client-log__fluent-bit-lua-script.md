---
source_image: docs_tutorials-evolution_list_topics_monitoring-and-logging__client-log__fluent-bit-lua-script.jpg
page_number: 0
model: model-run-olm-ocr
prompt_type: olmocr_technical
processing_time: 202.96
tokens: 21383
characters: 19055
timestamp: 2025-12-24T06:07:04.346938
finish_reason: stop
---
...
2. **Создайте сервисный аккаунт**. В блоке Доступы и роли выберите роли:
   ...
   - в блоке Сервисы — «logas.writer».
3. Для сервисного аккаунта создайте API-ключ. В параметрах API-ключа укажите сервис «logging_as_service».
...
<table>
  <tr>
    <th>Из дистрибутива</th>
    <th>Автоматизированная установка</th>
    <th>Установка вручную</th>
  </tr>
  <tr>
    <td colspan="3">Установите приложение Fluent Bit из сборки дистрибутива для вашей операционной системы.</td>
  </tr>
</table>
...
Active: active (running) since Tue 2023-03-11 15:48:23 UTC; 3h ago
Main PID: 16598 (fluent-bit)
Tasks: 8 (limit: 2323)
Memory: 5.4M
CPU: 0m
CGroup: /system.slice/fluent-bit-bin/fluent-bit -- //etc/fluent-bit/fluent-bit.conf
```

3. После проверки сервиса fluent-bit остановите его, чтобы далее настроить на совместную работу с Logas:
...
1. Создайте файл logas_format.lua для форматирования логов в формат сервиса «Клиентское логирование»:

```sh
sudo touch /etc/fluent-bit/logas_format.lua
```

2. Откройте созданный файл с помощью редактора nano:

```sh
sudo nano /etc/fluent-bit/logas_format.lua
```
...
local whitelist_hash = {}
for _, key in ipairs(whitelist) do
    whitelist_hash[key] = true
end

local json = function(s)
    ...
    local function encode_value(val)
        ...
        elseif t == 'table' then
            ...
            for k, v in pairs(val) do
                ...
                table.insert(result, key .. '=' .. encode_value(v))
            end
    ...
    return {
        encode = function(tbl) return encode_value(tbl) end
    }()
end()

function extra_fields(record)
    local message_data = {}
    local keys_to_remove = {}
    local message_data = {}

    for key, value in pairs(record) do
        if not whitelist_hash[key] then
            message_data[key] = value
            table.insert(keys_to_remove, key)
        end
    end

    if next(message_data) ~= nil then
        json_message = json.encode(message_data)
    else
        return nil
    end
    ...
    function ensure_string(var)
        ...
        if type(var) == "number" or type(var) == "boolean" then
    ...
    function format_log(tag, timestamp, record)
        local project_id = record.default_project_id
        local default_project_id = record.default_project_id
        local default_group_id = record.default_group_id
        local group_id = record.group_id
        ...
        record.default_project_id = nil
        record.default_group_id = nil

        -- 2. Timestamp
        local system_timezone = os.date("%z")
        local timezone_formatted = string.sub(system_timezone, 1, 3) .. ":" .. string.sub(system_timezone, 4, 6)
        local nsec = os.time()
        local iso_timestamp = os.date("%Y-%m-%dT%H:%M:%S", sec)
        local milliseconds = string.format("%03d", math.floor(nsec / 1000000))
        local formatted_ts = iso_timestamp .. "." .. milliseconds .. timezone_formatted
        ...
        if record.labels then
            for k, v in pairs(record.labels) do
                val, ok = ensure_string(v)
                if ok then
                    merged_labels[k] = val
                else
                    print("skip unsupported type value "..k.." => "..v)
                end
            end
        end
        ...
        local log_entry = {
            ...
            message = message,
            json_message = extra_fields(record)
        }

        if record.trace_id and record.trace_id ~= "" then
            log_entry.trace_id = record.trace_id
        end

        if record.service_name and record.service_name ~= "" then
            log_entry.service_name = record.service_name
        end

        if record.instance_id and record.instance_id ~= "" then
            log_entry.instance_id = record.instance_id
        end

        -- 6. Complete
        return log_entry, timestamp, log_entry
    end
end()
...
name modify
match *
Set default_labels.name_field_A value_A
Set default_labels.name_field_B value_B

[FILTER]
Name lua
Script logas_format.lua
Call format_log
time_as_table true
...
• REPLACE_TO_PROJECT_ID и REPLACE_TO_LOG_GROUP_ID — ID проекта и ID лог-группы, в которую будут отправлены логи. REPLACE_TO_LOG_GROUP_ID — необходимая строка. Если ее не добавить, логи отправятся в группу проекта по умолчанию (default-группа).

• REPLACE_TO_SA_API_KEY — API-ключ сервисного аккаунта. Проверьте, что у вас есть доступ к проекту, а для вашего сервисного аккаунта выбраны роли «Пользователь сервисов» и «logas.writer».
...
В следующем шаге инструкция настраивается тестовая отправка данных с помощью генератора логов, который записывает логи в лог-файл. Для тестирования с помощью генератора вместо <path-to-log/logfile.log> укажите путь к лог-файлу: /usr/local/bin/log_producer/error_log.log .
...
URI /api/v1/logs-ingest
json_date_key false
Header Authorization Api-Key ZDVhMwVlY2txxxxxxxxxxxxxxxxxhyTjN0UJ.kxxxxxxxxxxxxxx
```
...
generate_log() {
    # Generate timestamp in UTC with 6 fractional seconds
    nanoseconds=$(date +%s%N)
    trimmed_ns=$(echo $nanoseconds | cut -b1-6)
    timestamp=$(date -u +%Y-%m-%dT%H:%M:%S.$trimmed_nsZ)

    # Random log level selection
    levels=("TRACE" "DEBUG" "INFO" "NOTICE" "WARN" "ERROR" "CRITICAL" "ALERT" "EMERGENCY" "FATAL")
    ...
    # Logger name (fixed)
    logger="ru.rlabo.einfahrt.query.server.http.request.query.RQueryCaExecutorImpl"
    request_id=$RANDOM
    message="response received request $request_id got response $response"
    context="default"
    created_time=$(date "Europe/Moscow" date +"%Y-%m-%dT%H:%M:%S.%3NZ")

    # Build json output
    echo "{\"@timestamp\":\"$timestamp\",\"level\":\"$level\",\"thread\":\"$thread\",\"logger\":\"$logger\",\"message\":\"$message\",\"context\":\"$context\",\"@dc_json\":}"

    # Handle Ctrl+C
    trap 'echo "\nLogging stopped. Output: $LOG_FILE"; exit' SIGINT
    echo "Logging to $LOG_FILE - Press CTRL+C to stop"
    while true; do
...
Последние строки кода запускают генератор логов в бесконечном цикле — чтобы остановить генератор, нажмите CTRL + C . Вы можете изменить это поведение генератора — например, чтобы задать генерацию логов в течение 1 минуты, замените строки:
